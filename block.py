...
def filetoblock(filename) :				
	...
	for i in range(B.max_trans_num) :
		...
		T.inlist = [transaction.inputtrans() for i in range (T.incount)]	#Create an array of type inlist[incount]
		T.sign = f.readline()
		T.hash = f.readline() 
		...
		T.outlist = [transaction.outputtrans() for i in range (T.outcount)]	   #Create an array of type outlist[outcount] 
